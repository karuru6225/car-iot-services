...
### フォントサイズ設定（WCAG 1.4.4 Resize text）への耐性

端末の「フォントサイズ」を大きくした状態でも読めることを確認する。エミュレータなら
`adb shell settings put system font_scale 1.5`（戻すときは `1.0`）で再現できる。
コントラスト比と違って計算では出ないので、実際に拡大して目で見るしかない。

1.5倍で実際に見つかった不具合:

- **数値が黙って切り捨てられていた**。`maxLines = 1` の既定の overflow は `Clip` なので、
  `13.812` が `13.81` に、`-116.72` が `-116.7` になり、省略記号も出ない。
  **崩れて見えるのではなく「別の正しそうな値」に見える**ため、計測値の表示としては
  誤読を生む。`AutoSizeValueText`（`BasicText` + `TextAutoSize.StepBased`）で
  枠に収まるまで縮小し、全桁を残すようにした
- **アナログダイヤルの目盛りの数字だけ拡大しなかった**。Canvas に直接描く
  `android.graphics.Paint` の `textSize` に `dp` を渡していたため。**文字サイズは必ず
  `sp`** で指定する。ただし文字盤は直径が決まっているので、半径に対する上限で
  頭打ちにしないと目盛りを突き破る
