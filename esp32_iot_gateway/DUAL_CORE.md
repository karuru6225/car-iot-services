...
**未実装**（設計検討のみ）。ESP32-S3 の 2 コアを活かした 3 つの改善案。それぞれ独立して実装できる。着手する場合は本書を出発点にする。
...
### 現在の起動シーケンス（`main.cpp` の `setup()` / `loop()`）
...
setup():
  ...
  adsInit()
  ina228.init()
  speakerInit() / playMelody(bootStart)  ← 電源ON時のみ
  button.begin()
  bleScanner.setup()   ← BLE スタック初期化（本番・デバッグ共通）
  bleTargets.load()
  [BTN0 起動時] enterMenuMode()
  lte.setup()          ← ~20-40秒（GPRS接続・時刻同期・TLS証明書設定）
  queue.load() / queue.flush()
  ota.reportPendingJobResult()
  ota.confirmBoot()    ← MQTT 接続確認時のみ
  shadowSetup() / shadowPollDelta()
  jobsSetup() / jobsGetNext()  ← OTA or コマンド処理

loop():
  monitor.measure()    ← BLE スキャン(10秒) + ADS1115/INA228 読み取り（本番・デバッグ共通）
  monitor.publish()    ← MQTT publish
  [DEEP_SLEEP] → shadowPublishConfig() → lte.disconnect() → lte.radioOff() → DeepSleep
  [CONTINUOUS] → 5分待機（ボタン監視）→ 繰り返し
```

BLE スキャンは本番モード（DEEP_SLEEP）でも毎サイクル実行される。`bleScanner.start()` は `monitor.measure()` 内でブロッキング呼び出しされる。
...
```
Core 1 (setup):
  bleScanner.setup() / bleTargets.load()
