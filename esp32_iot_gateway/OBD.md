...
**所見（4サンプル後。燃料が実際に動いた区間を初めて観測）:**

- `0x2341`/`0x2342`/`0x2601`/`0x2630`/`0xE5FF`/`0xF100`/`0xF806`の7件は、燃料が実際に9/10→8/10へ
  減った区間を含めても**一貫して不変**。この7件は燃料残量DIDから除外してよい。
- `0xE600`は`14280→14292→14255→14192`。アイドルのみの区間では小さく上下していた（+12, -37）のに対し、
  燃料が実際に減った走行区間で最大の下げ幅（**-63**）を記録し、方向（減少）が燃料消費と一致する。
  **現時点で最有力な燃料残量候補**。
- `0xE602`は16bit実効部（下位2バイト、3バイト目は常に`00`）で見ると`0EC4(3780)→0EEC(3820,+40)
  →0EEC(3820,+0)→0F24(3876,+56)`。アイドルのみ区間で伸びが鈍化した後、走行区間で再加速しているが
  増加方向のため燃料消費との対応は直接的ではない（走行距離・累積燃料消費量等の可能性は残る）。
- 水温は4回とも87°Cで変化しておらず、油温候補としての切り分けはまだできていない。次は**始動直後
