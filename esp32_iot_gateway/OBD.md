...
- `0x2341`/`0x2342`/`0x2601`/`0x2630`/`0xE5FF`/`0xF100`/`0xF806`は18分間のアイドリングで**一貫して不変**。
  フラグ/ステータス系（`0x2341`系）、固定値/ID系（`0xF100`・`0xF806`）という見立てを維持。
- `0xE600`は`14280→14292→14255`と**増加後に減少**しており、単調増加する累積カウンタ説は誤りと判明。
  水温・rpm・loadがほぼ一定のままこの値だけ数十単位で上下しているため、**ノイズを持つ瞬時値**
  （何らかのセンサーの生値、または車両状態と無関係な変動値）である可能性の方が高いと見立てを修正。
- `0xE602`は1→2回目で`C4→EC`（+40）動いたが2→3回目は不変。単調増加でも完全固定でもない中途半端な
  挙動で、現時点では性質を特定できない。
- 燃料残量・水温ともほぼ一定の条件が18分続いただけなので、これらのDIDが燃料残量・油温と無関係と
  結論づけるにはまだ早い。次は**燃料残量とエンジン水温が乖離するタイミング**（給油直後・始動直後）
  で読み、動き方が変わるDIDがないか確認する。
