...
**現時点の消去法的結論**: `0xE600`はギア位置・CVT比・アクセル開度・燃料残量・油温のいずれとも
系統的に対応しない。唯一確認できた反応は「エンジン始動直後のrpm収束（fast idle→通常アイドル）」
のみ。次に検証するなら、意図的にエンジン再始動直後の状態を作って（アイドリングが落ち着くまでの
数十秒）`0xE600`の挙動を再現できるか確認するのが筋が良さそう。

**次の検証案**: 安全な場所（駐車場等）に停車したまま、P→R→N→D→Lの順にシフトしてそれぞれの
タイミングをメモしながら`log2file`でログを取る。スイッチの生値なら各ギアでシフト直後に瞬時に
値が切り替わるはずなので、**切り替わりが瞬時か・数十秒かけて収束するか**を見ればこの2つの仮説を
切り分けられる。

**結論の更新**: `0xE602`はRPM、他7件はフラグ/ID系と確定。**燃料残量・油温に該当するDIDは、
今回の9候補の中には存在しない可能性が高い**（`0xE600`が唯一残った未解明枠だが、燃料残量・油温
のどちらの挙動パターンとも一致しない）。全域スキャンで発見済みのDIDは尽きているため、次に
