...
- `kDidCandidates[]`各DIDの実値と燃料残量・油温の対応付け（DID Values機能で値は読めるようになったが、意味の特定はこれから）
...
- `0x2341`/`0x2342`/`0x2601`/`0x2630`/`0xE5FF`/`0xF100`/`0xF806`は10分間のアイドリングで**完全に不変**。
  フラグ/ステータス系（`0x2341`系）、固定値/ID系（`0xF100`・`0xF806`）という1回目の見立てを補強する結果。
- `0xE600`が`37C8`→`37D4`で**+12**、`0xE602`が中位バイトで`C4`→`EC`と**+40**、10分間ずっと単調増加した
  （唯一動いたのがこの2件）。アイドリング中に水温は既に87-88°Cで飽和済み・燃料残量も10分のアイドリングでは
  ほぼ動かないはずの条件のため、瞬時値としての燃料残量・油温というより**時間や燃料消費で増え続ける
  累積カウンタ**（エンジンランタイム・燃料消費量・距離相当の内部カウンタ等）の可能性が高いと見立てを修正。
  増加率（10分で+12 / +40）から、そのまま燃料残量・水温の代替として使うのは難しそうだが、
  累積カウンタなら差分から燃料消費量を逆算できる可能性はある。
- 燃料残量そのもの・油温そのものに該当するDIDは、この2サンプルの範囲では依然未確定。
  次は**燃料残量とエンジン水温が乖離するタイミング**（給油直後・始動直後）で読み、
  瞬時値らしい動き方をするDIDが他にないか確認する。
...
**次回サンプリング計画（未実施）**: 1回の外出で複数ポイントを撮れると効率が良い。

1. 出発前（冷間・燃料は現状維持〜満タン付近）
2. 走行後しばらくして（水温上昇、燃料はほぼ不変）
3. 給油直後（燃料だけジャンプ、水温は無関係に推移）

この3点が揃えば「水温と連動する値」「燃料と連動する値」「どちらとも無関係な値」に分類できる。
給油タイミングまで待てない場合は①②だけ先に取ってもよい。
