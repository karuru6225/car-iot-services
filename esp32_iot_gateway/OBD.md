...
- 未完成コマンド（`\r`が来る前にバイトが途切れた状態）に200msのアイドルタイムアウトを追加
  （2026-08-25）。PC側の切断・クラッシュ等で中途半端なバイト列が`s_cmdBuf`に残ったままだと、
  再接続後に送られてきた正常なコマンドの先頭に紛れ込んで誤解釈される実害があったための対策
- コマンド終端の`\n`（CRLF送信対策）を無視するようにした（2026-08-25）。`\r`のみで終端する
  仕様だが、多くのシリアルライブラリは`\r\n`で送ってくるのが一般的で、`\r`直後の`\n`が次の
