...
void Lte::uploadCert(const char *filename, const char *pem)
{
  ...
  while (millis() - t < 5000) {
    if (SerialAT.available()) {
      ...
      if (buf.indexOf("DOWNLOAD") >= 0) { gotPrompt = true; break; }
    }
  }
  if (!gotPrompt) {
...
bool Lte::publish(const char *topic, const char *payload)
{
  ...
  while (millis() - t < 3000) {
    if (SerialAT.available()) {
      buf += (char)SerialAT.read();
      if (buf.indexOf('>') >= 0) { gotPrompt = true; break; }
    }
  }
  if (!gotPrompt) {
                   ...
                   uint32_t timeoutMs)
{
  ...
  while (millis() < deadline) {
    ...
    int nl = buf.indexOf('\n', comma + 1);
    if (q1 < 0 || q2 < 0 || comma < 0 || nl < 0) { delay(10); continue; }
...
statusDone:
  ...
  while (millis() - t < 15000) {
    while (client.available()) {
      char c = client.read();
      if (c == '\n') {
        if (line.length() == 0) goto headerDone;
        line = "";
      } else if (c != '\r') {
        line += c;
      }
...
bool Lte::syncTime()
{
  ...
  if (q1 < 0 || q2 <= q1) { logger.println("[TIME] CCLK パース失敗"); return false; }
...
void Lte::setup()
{
  ...
  sendCmd("AT+CSCLK=0");
  for (int i = 0; i < 5; i++) {
    if (sendCmd("AT")) { logger.println("[LTE] モジュール応答 OK"); break; }
  ...
  while (millis() - t0 < 15000) {
    if (SerialAT.available()) {
      ...
      if (line.indexOf("SMS Ready") >= 0) { logger.println("[LTE] SMS Ready!"); break; }
    }
  }
