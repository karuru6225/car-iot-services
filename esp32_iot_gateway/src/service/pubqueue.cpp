...
void PubQueue::load()
{
  if (g_magic == RTC_MAGIC) return;  // DeepSleep 復帰: RTC メモリ有効

  // 電源投入: RTC 初期化
  g_head = g_tail = g_count = 0;
  g_magic = RTC_MAGIC;

  if (!_useSpiffs) return;
  ...
  uint32_t magic  = 0;
  uint16_t count  = 0;
  if (f.read((uint8_t *)&magic, 4) != 4 || magic != RTC_MAGIC) { f.close(); return; }
  if (f.read((uint8_t *)&count, 2) != 2)                        { f.close(); return; }
