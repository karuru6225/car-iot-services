...
SORACOM SIM の通信コスト削減を目的に、MQTT ペイロードのサイズを圧縮する。

#### 方針 1: フィールド名短縮

現行の JSON キー名を短い別名に変更する。クラウド側 Lambda（ingest）と Athena スキーマも合わせて変更が必要。
...
例: `{"t":"battery","m":12.34,"s":12.10,"i":5.21,"p":62.5,"tp":28.5,"ah":200.001,"ts":1746143400}`（約 80 bytes → 現行 約 120 bytes）

#### 方針 2: ペイロード gzip 圧縮

`lib/uzlib/` の圧縮 API をそのまま使用。MQTT publish 前にペイロードを gzip 圧縮して送信する。
AWS IoT Core の Topic Rule / Lambda 側で解凍処理が必要。

**注意**: JSON 80 bytes 程度では圧縮効率が低く（圧縮後が大きくなる可能性あり）、フィールド名短縮との組み合わせが前提。BLE センサーデータが多い場合（複数台登録時）には効果が出やすい。

**実装順序（推奨）**: フィールド名短縮を先に行い、効果を実測してから圧縮の要否を判断する。

#### 方針 3: MessagePack 等のバイナリ形式

JSON の代わりに MessagePack（または CBOR）でエンコードする。フィールド名短縮と組み合わせることでさらに削減できる。

**削減効果の試算**（バッテリーテレメトリ 1 件、MQTT フレーミング込み）:

| 形式 | ペイロード | MQTT込み合計 |
| --- | --- | --- |
| JSON（現行） | ~120 bytes | ~156 bytes |
| JSON + フィールド名短縮 | ~96 bytes | ~132 bytes |
| MessagePack + フィールド名短縮 | ~67 bytes | ~103 bytes |

※ MQTT オーバーヘッド ~36 bytes（固定ヘッダ + トピック名）は共通。
※ フロート値は float32 で格納。`ah` のみ精度確保のため float64 が必要。

##### JSON との比較: 桁数の少ない浮動小数点はバイナリ優位でない

JSON テキストは値が小さいほどコンパクト（`5.21` = 4 bytes）なのに対し、MessagePack float32 は型タグ込みで最低 5 bytes 固定。電圧・電流のような小さい値では差が出にくい。MessagePack が稼げるのは主に Unix タイムスタンプ（10 chars → uint32 5 bytes）と構造文字の削除。それでも MQTT フレーミング込みで **~34% 削減**になる。

##### gzip との併用

MessagePack は冗長性を削った後なので gzip による追加圧縮効果は低い。1 レコード単体では gzip ヘッダ（18 bytes 固定）がオーバーヘッドになる。gzip は複数レコードのバッチ送信時にのみ検討する。

##### 課題

- ESP32 側に MessagePack ライブラリが必要（ArduinoJson v7 の `MsgPackDocument` が候補。現行バージョンとの互換性確認が必要）
- AWS IoT Core の Topic Rule は JSON しかパースできない → 別トピック（`sensors/+/data_bin`）+ 新 Topic Rule を追加し、Lambda で MessagePack デコード → S3 に JSON 保存する構成にする
- 移行は「Lambda + 新 Topic Rule を先にデプロイ → OTA でファーム更新」の順序を守る（ファーム先行は禁止）
- **`AT+SMPUB` のバイナリペイロード対応要確認**: 現行の `Mqtt::publish()`（`service/mqtt.cpp`）は `strlen()` + `SerialAT.print()` でペイロードを送っており `\0` で打ち切られる。MessagePack は `\0` を含むため `publishBinary(topic, uint8_t*, size_t)` を追加し `SerialAT.write(buf, len)` に変える必要がある。SIM7080G は `AT+SMPUB=<topic>,<len>` で長さを明示する設計なのでバイナリ送信できる可能性が高いが、マニュアルに明記がないため**実機検証が必要**。
