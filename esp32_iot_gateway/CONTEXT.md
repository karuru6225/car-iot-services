...
```json
{"state":{"reported":{"ah_offset":200,"chg_start_v":11.70,"chg_stop_v":12.50,"chg_min_diff_v":0.30,"charging":false,"override_next_mode":null,"continuous_until_time":null,"fw_version":"1.24.0+xxxxxxxx"}}}
```
...
#### 引き継ぎサマリー（2026-08-19時点、ここから下は検証の詳細ログ）

**現状**: `trip_analysis`機能自体（バックエンドLambda・Web管理画面`web/trip-analysis.html`）は実装済み・PR #35でmainにマージ・実運用中（実車データが日々溜まっている）。**AIナレーティブ生成本体はまだ未実装**（`narrative`は常に空文字）。まる1日かけて`experiments/trip-analysis-ai-poc/`（gitignore対象）でBedrockへの実投入テストを繰り返し、プロンプト設計の方針が固まった。

**確立した設計方針（実装時に踏襲すること）**:

1. **モデルはBedrock Nova Pro**（`apac.amazon.nova-pro-v1:0`、inference profile経由必須）。個人利用スケールなら1トリップ約1円、月100回呼んでも百数十円で無視できるコスト
2. **数値の異常判定は絶対にAIに委ねない。コード側でz-score/偏差値を計算し、判定済みの自然言語ラベル（「やや薄め」「大きく高め」等、`FIELD_LABELS`マッピング）に変換してから渡す**。固定しきい値だとオオカミ少年化し、基準値を渡さないフィールドはAIが架空の閾値を作って誤検知し、平均と最大のような異なる統計量をAIが取り違えることが、実データで繰り返し確認された
3. **ベースラインは車固有（過去トリップの`row_count`重みつき平均・標準偏差）を主とし、一般的な絶対評価の目安（例: 軽ターボ車の燃費15〜20km/L）も併記する**。車固有だけだと「そもそも一般的にどうか」という素朴な感覚が抜け落ちる
4. **ベースライン計算時は`distance_km`の下限フィルタが必須**（GPS未捕捉による"幽霊トリップ"がfuel_economy等を大きく歪めるため）
5. **位置情報は生の緯度経度をAIに渡さず、AWS Location Service（データソースは`Here`、`Esri`は日本のPOI精度が低く不可）で逆ジオコーディングしてから文字列で渡す**。自宅は固定座標+半径判定で「自宅」と匿名化する
6. **時系列データの特定区間への言及は秒数でなく`{lat:xxx,lon:yyy}`座標マーカー形式**にすると、後で機械的に地図と紐づけられる
7. **提案・アドバイスは「④提案」の独立枠に隔離**し、①②③本文には書かせない（さもないと散発的に混入する）
8. **入力データの品質チェックもコード側の責務**: イグニッションOFF直前・アイドリングストップ切替瞬間に、複数センサー値が同時にゼロになる/桁外れの異常値になる通信断パターンが実データで確認されている。集計前に除外するバリデーションが必要
9. レポートは「④提案」以外では一般論的な警告を極力書かせない指示を入れても、完全にはゼロにならない（頑固な残課題）

**副産物**: 検証中に発見した「モバイルアプリのOBDダッシュボードでグラフが画面を貫く縦線に見える」不具合（アイドリングストップ切替瞬間にPID 0x43デコードが桁外れの値になる）は、`_pushHistory()`にメタのmin/max範囲チェックを追加して**PR #36で修正・マージ済み**

**次にやること**: `infra/lambda_src/trip_analysis/index.py`の`_process_job()`に、上記方針に沿った実装（バケット化→ラベル化→ベースライン計算→Bedrock呼び出し→`narrative`保存）を追加する。実行可能な試作コード一式は`experiments/trip-analysis-ai-poc/`（README.mdに詳しい手順あり）を出発点にできる。

**詳細な検証過程（時系列順の試作ログ、上記サマリーの元ネタ）は`experiments/trip-analysis-ai-poc/DESIGN_LOG.md`に分離した**（`.gitignore`対象。設計方針を素早く把握したいだけならこの先を読む必要はない）。
