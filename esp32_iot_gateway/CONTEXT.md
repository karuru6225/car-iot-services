...
| namespace | キー | 用途 |
| --- | --- | --- |
| `"device"` | `mqtt_host`, `relay_mode` | MQTT ホスト、リレー動作モード |
| `"lte"` | `cert_crc` | 証明書 CRC |
| `"ota"` | `job_id` | 保留中 OTA ジョブ ID |
| `"battery"` | `ah_offset`, `chg_timeout`, `chg_start_v`, `chg_stop_v`, `chg_duration` | Ah オフセット、充電タイムアウト（分）、充電開始電圧（V）、充電停止電圧（V）、自動充電継続時間（秒） |
...
| 定数 | 値 | 用途 |
| --- | --- | --- |
| `FIRMWARE_VERSION` | `"1.9.1+" GIT_HASH` | ファームウェアバージョン |
