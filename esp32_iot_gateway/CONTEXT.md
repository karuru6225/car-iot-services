...
1. ~~**接続検証**~~（完了）: NUS → カスタムサービスへ移行し双方向通信を確認
2. ~~**計測値ダッシュボード**~~（検証完了）: 電圧・電流・電力・温度を Notify で受信・表示
3. ~~**設定 Read/Write**~~（検証完了）: 低電圧アラート閾値・スリープ間隔の読み書きを確認
4. **本番実装**: `ble_verify.cpp` の GATT 設計を `device/ble_peripheral.h/.cpp` として本体に組み込む
5. **DeepSleep + GPIO0 wakeup** の組み込み

#### 検証済み GATT 構成

参照実装: `src/ble_verify.cpp`、Flutter: `mobile/lib/main.dart`
...
| 電圧 | `f3a8b2c2-...` | float32 | V |
| 電流 | `f3a8b2c3-...` | float32 | A |
| 電力 | `f3a8b2c4-...` | float32 | W |
| 温度 | `f3a8b2c5-...` | float32 | °C（INA228 ダイ温度） |

**設定サービス** — Read / Write（with response）

| Characteristic | UUID | 型 | 内容 |
| -------------- | ---- | -- | ---- |
| 設定サービス | `f3a8b2d1-d4e5-4f6a-7b8c-9d0e1f2a3b4c` | — | Service |
| 低電圧アラート閾値 | `f3a8b2d2-...` | float32 | V（デフォルト 11.0） |
| スリープ間隔 | `f3a8b2d3-...` | uint32 | 秒（デフォルト 300） |

#### DeepSleep 解除: BOOT ボタン（GPIO0）

通常の DEEP_SLEEP モードは5分周期で完結するため、スマホからの BLE 接続窓がない。  
基板上の BOOT ボタン（SW1 / GPIO0）を押すと DeepSleep から復帰し、BLE モードに入る設計にする。

| 項目 | 内容 |
| ---- | ---- |
| ウェイクアップ方式 | `esp_sleep_enable_ext0_wakeup(GPIO_NUM_0, 0)`（LOW = ボタン押下） |
| ストラッピング再サンプリング | DeepSleep 復帰は RTC reset のため GPIO0=LOW でも download モードに入らない |
| 動作フロー | BOOT ボタン押下 → DeepSleep 解除（EXT0） → `setup()` で起床要因を判定 → BLE advertise 開始 → 接続・データ配信 → 切断 or タイムアウトで再び DeepSleep |

#### 本番 FW 実装方針

- `device/ble_peripheral.h/.cpp` を新設し `ble_verify.cpp` の GATT 構成をそのまま移植する
- `main.cpp`: DeepSleep 前に `esp_sleep_enable_ext0_wakeup(GPIO_NUM_0, 0)` を追加。`setup()` で `ESP_SLEEP_WAKEUP_EXT0` を判定し BLE モード分岐
- `device/ble_scan.cpp`: `NimBLEDevice::deinit(true)` をやめ、BLE を維持したまま Peripheral を起動できるよう整理
- release env の `CONFIG_BT_NIMBLE_ROLE_PERIPHERAL_DISABLED=1` を削除する（BLE ダッシュボード表示器 TODO と共用）
- 設定値は `config.cpp` の NVS 操作パターンに倣って永続化する
