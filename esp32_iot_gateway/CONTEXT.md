...
1. **ダッシュボード機能（最初に実装）**: リアルタイムセンサーデータ（電圧・電流・電力・BLE センサー値）をスマホで表示する。読み取り専用。
2. NVS 設定値の読み書き（低電圧アラート閾値など）
3. 既存の Web ダッシュボードに BLE 接続ボタンを追加する形で統合
...
- Nordic UART Service（NUS）を BLE GATT Server として実装（Notify で JSON を送信）。
