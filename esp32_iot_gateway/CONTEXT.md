...
### TODO: trip_analysisの通信断ギャップで燃費が破綻するケースがある（未修正、報告のみ）

2026-08-22のトリップ`car-iot-aca704`の`trip-analysis/`結果に、距離2.03km・燃料消費0.005L・燃費372km/Lという物理的にありえない値が見つかった（`1787381032_1787381327_v01_001.json`、15:43〜15:48）。

**原因**: 生のOBDデータ（`obd/year=2026/month=08/day=22/hour=06/`）を確認したところ、このトリップの途中で255秒間データが完全に欠落しており、その欠落区間の直前・直後の行がどちらも通信断（`coolant_c==0 かつ ecu_voltage==0`、`fuel_rate_lph`も0）だった。`_compute_summary()`の台形積分は`prev`/`cur`間の`dt`をそのまま使うため、この255秒間の燃料消費が積分上ゼロ扱いになった。一方GPS座標（`lat`/`lon`）は途切れず前後の行に残っていたため、この間に約2km移動したものとして距離だけは正しく（あるいはGPSジャンプの可能性もあり、判別できていない）積算された。結果、「距離はあるのに燃料がほぼゼロ」という不整合が生まれた。

**この問題は一つのトリップだけの外れ値ではない**。前後のトリップ（同日13:18-15:32、16:15-16:25）も確認したところ、どちらも通信断に隣接する時間ギャップ（421秒・293秒・59秒）が存在した。ただしこれらはギャップの間ほぼ停車していた（GPS座標がほとんど動いていない）ため、燃費が破綻するほどの実害は出ていない。**通信断に隣接するギャップの間、燃料消費が積分上ゼロ扱いになる現象自体はほぼ全トリップで起きているが、たまたまその間に実移動（またはGPSジャンプ）が重なった場合にだけ燃費が明らかにおかしい値として表面化する**、という構造。

**今回AIナレーティブ生成用に追加した`_is_comm_dropout()`はcoolant_c/ecu_voltageの集計からのみ通信断行を除外しており、この距離・燃料の積分ループには適用されていない**。

**修正方針（未確定、要検討）**:

- 案1: `prev`/`cur`のどちらかが`_is_comm_dropout()`ならその区間の距離・燃料を両方とも積算から除外する（最も保守的。トリップBの2kmが実移動かGPSジャンプか判別できない以上、疑わしい区間は距離も燃料も除外するのが安全）
- 案2: 燃料のみ除外し、距離はGPS座標の差分として信頼する（GPS自体は途切れていないため）

**次にやること**: 上記いずれかの方針で`_compute_summary()`の距離・燃料積分ループを修正し、既存のtrip_analysisテストスイートにこのギャップパターンの回帰テストを追加する。
