...
**実際に新しく記録されたトリップ（2026-08-19 19:26:28〜19:44:02、17分34秒・5.92km）を分析させた
ところ、実データならではの重大な問題が見つかった（2026-08-19）**:

- このトリップの末尾5行（10秒分）で、`coolant_c`（冷却水温）と`ecu_voltage`（ECU電圧）と`rpm`と
  `speed_kmh`が**全て同時に0**という値になっていた。エンジン停止直後（イグニッションOFF直後）に
  OBDアダプタの電源が落ちかけ、最後の読み取りフレームが全項目ゼロ埋めされた通信断と見られる
  （`rpm=0`単体はエンジン停止として自然だが、走行中を通じて74°C前後だった冷却水温や12〜14V台
  だったECU電圧が最後だけ0になるのは物理的にありえない）
- **これをそのままAIに渡した結果、AIは「冷却水温は開始時74°Cから終了時0°C、ECU電圧は開始時
  12.05Vから終了時0.00Vでした」と、異常性を一切疑わずにサマリーへ数値のまま列挙した**。今までの
  「AIが数値を捏造/誤解釈する」問題とは逆方向の、**実データの欠陥をAIが無批判に受け入れて
  報告してしまう**という新しいパターン。ユーザーが見れば「バッテリー上がり？エンジン故障？」と
  誤解しかねない、実害のある問題
- **対策（コード側）**: `coolant_c == 0 かつ ecu_voltage == 0`が同時発生する行を、通信断による
  無効データとして集計（`coolant_c`/`ecu_voltage`のstart/end/mean算出）から除外するフィルタを
  実装した。この機械的な条件（2つの独立したセンサー値が同時にゼロという物理的にありえない
  組み合わせ）で判定するため、実際の異常値（本当に電圧が下がっている等）を誤って除外するリスクは
  低い。除外後は冷却水温74→87°C・ECU電圧12.05→14.28Vという正常な値になり、AIのレポートからも
  異常値への言及が消えた
- **教訓**: これまでの検証は主に「AIにどう数値を解釈させるか」に焦点を当ててきたが、**入力データ
  自体の品質チェック（通信断・センサー欠損によるゼロ埋め等の異常値除外）も同様にコード側の責務**
  として実装時に組み込む必要がある。実装時は`_compute_summary()`や集計処理の前段で、この種の
  「複数センサー値が同時にゼロになる」パターンを検出・除外するバリデーションを追加すべき
