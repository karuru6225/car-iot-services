...
TODO起票時に想定していた「マージ済みファイルならGetObject→対象行を除いて残りをPutObjectで上書き」という行単位の精密な削除ではなく、**一致行が1件でもあればファイル全体を削除する**（他の非一致行も道連れになる）簡易実装を採用した。起票時の問題（NDJSONファイルで`json.loads`に失敗し何も削除できていなかった）自体はこれで解消しているため、精密な行単位削除は現状オーバースペックと判断し対応不要とした。
