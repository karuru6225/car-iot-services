...
3. `AT+HTTPTOFS` で S3 からファームウェアを SIM7080G ファイルシステム（`/customer/firmware.bin`）にダウンロード
4. `esp_ota_begin()` で書き込み開始
5. `AT+CFSRFILE` で 4096 バイト単位チャンク読み取り → `esp_ota_write()` に渡す
6. `esp_ota_end()` で検証（SHA256 チェック）
7. `esp_ota_set_boot_partition()` で次回起動先を新スロットに設定
8. SIM7080G ファイルシステムから `firmware.bin` を削除（`AT+CFSDFILE`）
9. `esp_restart()` で再起動（再起動後に SUCCEEDED を更新）

### ダウンロード

- `AT+HTTPTOFS` で S3 から SIM7080G ファイルシステムに保存後、`AT+CFSRFILE` で 4096 バイト単位チャンク読み取り
- MQTT（AT+SM*）と独立した HTTP スタックを使用（MQTT 接続中でも利用可能）
- S3 仮想ホスト URL（`bucket.s3.region.amazonaws.com`）で動作確認済み（URL 長制限なし）
- client cert 不要（CA 証明書のみで TLS 検証）
