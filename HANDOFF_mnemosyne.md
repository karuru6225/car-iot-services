...
**未着手**。決まっているのは方針と、ネモ側が受け取れる形（下の2章）だけ。

| 段階 | 内容 | 担当 |
|---|---|---|
| ① | 読み取り専用の MCP サーバを作る（会話中に車のデータを引ける状態にする） | このリポジトリ |
...
## 6. 未決定事項（実装前にユーザーに確認する）

1. **AWS の認証**: 読み取り専用の IAM ユーザーを新しく作るか
2. **位置情報**: 地名まで会話モデルへ渡してよいか
3. **実装言語と依存**: Lambda 群と同じ Python にするなら MCP の Python SDK（`mcp`）が新しい依存になる。
   Node.js にするならネモと同じ `@modelcontextprotocol/sdk`。どちらにするか、新規依存の追加の可否
4. **置き場所**: リポジトリ内のディレクトリ名（例: `mcp_server/`）
5. **イベントの閾値**: 2章の表の値。過去データの再生結果を見てから決める
