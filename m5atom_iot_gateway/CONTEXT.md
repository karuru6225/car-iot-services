...
│   ├── platformio.ini                     PlatformIO ビルド設定（C++17 有効）
│   ├── src/
...
```json
{"type":"thermometer","addr":"AA:BB:CC:DD:EE:FF","temp":25.0,"humidity":60,"battery":80,"rssi":-70,"mf":"09b0e9...","ts":"2026-03-05T12:00:00Z"}
```

### CO2（co2meter タイプ）

```json
{"type":"co2meter","addr":"AA:BB:CC:DD:EE:FF","temp":25.0,"humidity":60,"co2":612,"battery":80,"rssi":-70,"mf":"09b0e9...","ts":"2026-03-05T12:00:00Z"}
```

`mf` フィールドは Manufacturer Data の生バイト列を小文字 hex 文字列化したもの（デバッグ用途）。

ingest Lambda で `sensor_type` フィールドを付加して S3 に保存する（`"voltage"` / `"switchbot"` / `"switchbot_co2"`）。
...
| `sensor_type` | string | `"voltage"` / `"switchbot"` / `"switchbot_co2"` |
| `voltage` | double | 電圧（V） |
...
※ `co2` / `mf` カラムはクラウド側未実装（terraform apply 要）
...
| `PAYLOAD_BATTERY_SIZE` | `96` | battery ペイロードバッファサイズ（バイト） |
...
デバイス側は実装・動作確認済み。以下がクラウド側の残作業:

| ファイル | 変更内容 |
| --- | --- |
| `infra/lambda_src/ingest/index.py` | `type == "co2meter"` → `sensor_type = "switchbot_co2"` の分岐を追加 |
| `infra/s3.tf` | Glue テーブルスキーマに `co2 int` / `mf string` カラムを追加 → `terraform apply` 必要 |
| `infra/lambda_src/query/index.py` | `switchbot_co2` クエリ分岐追加、UNION ALL に含める |
