...
void loop()
{
  ...
  // ─── デバッグモード: 送信 → 待機 ─────────────────────────────────────
  {
    ...
    char payload[PAYLOAD_BATTERY_SIZE];
    snprintf(payload, sizeof(payload),
             "{\"type\":\"battery\",\"id\":\"voltage_%d\",\"voltage\":%.2f,\"fw\":\"" FIRMWARE_VERSION "\"%s}", 1, voltage, tsField);
  ...
  while (xQueueReceive(scanner.queue, &v, 0) == pdTRUE)
  {
    std::visit([&](auto &d)
      ...
      {
        view.thermometerData(d);
        snprintf(payload, sizeof(payload),
                 "{\"type\":\"thermometer\",\"addr\":\"%s\",\"temp\":%.1f,\"humidity\":%d,\"battery\":%d,\"rssi\":%d,\"mf\":\"%s\",\"fw\":\"" FIRMWARE_VERSION "\"%s}",
  ...
  static char payloads[MAX_TARGETS + 1][PAYLOAD_SENSOR_SIZE];
  int count = 0;
  {
    float voltage = readVoltage();
    logger.printf("[ADS] 電圧: %.2f V\n", voltage);
  ...
  Wire.end();
  {
    ...
    io_conf.pin_bit_mask = (1ULL << LTE_TX_PIN) | (1ULL << LTE_RX_PIN) | (1ULL << SDA_PIN) | (1ULL << SCL_PIN);
