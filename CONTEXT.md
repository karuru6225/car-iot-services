...
ESP32-S3
  ...
  → AWS IoT Core  topic①: $aws/things/{device_id}/shadow/update  （シャドウ更新）
                  topic②: sensors/{device_id}/data               （BLE センサーデータ）
  → Topic Rule①: SELECT *, topic(2) AS device_id FROM 'sensors/+/data'
    → Lambda ingest → S3: raw/.../{device_id}-{uuid8}.json
  → Topic Rule②: SELECT state.reported.*, 'shadow' AS type, topic(3) AS device_id
