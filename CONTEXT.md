...
ESP32-S3
  ...
  → Topic Rule ingest_bin: SELECT encode(*,'base64') AS payload, topic(2) AS device_id FROM 'sensors/+/data_bin'
    → Lambda ingest（base64 decode → msgpack decode）→ S3: raw/.../{device_id}-{uuid8}.json
...
トピック: `sensors/{device_id}/data_bin`（**v1.15.0 以降 MessagePack 形式**）

通信経路上のフィールド名は短縮形を使用。`ingest` Lambda が base64 decode → MessagePack decode → キー展開して S3 に保存する。実測ペイロード: **59 bytes**（旧 JSON ~96 bytes → 約 38% 削減）。
...
```json
{"state":{"reported":{"ah_offset":200,"relay_mode":"sleep_indicator","chg_start_v":11.70,"chg_stop_v":12.50,"fw_version":"1.15.0+xxxxxxxx"}}}
```
