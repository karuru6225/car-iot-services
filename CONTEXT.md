...
```
car-iot-services/
├── esp32_iot_gateway/                     ESP32-S3-MINI-1 ゲートウェイ（PlatformIO プロジェクト）
│   ├── platformio.ini                     ビルド設定（SPIFFS 有効・QIO パッチ）
│   ├── extra_scripts.py                   ビルドフック: git hash 注入・firmware.bin を QIO パッチ
│   ├── sdkconfig.defaults                 OTA ロールバック有効化
│   ├── partitions_two_ota.csv             デュアルバンク OTA パーティション構成（8MB）
│   ├── OTA.md                             OTA 仕様ドキュメント
│   ├── data/certs/                        SPIFFS 証明書置き場（gitignore。provision_device で書き込む）
│   └── src/
│       ├── main.cpp                       起動 → LTE → OTA チェック → DeepSleep
│       ├── config.h                       定数（SPIFFS パス・MQTT ポート・スリープ間隔）
│       ├── certs.example.h                証明書テンプレート（現在はコメントのみ。秘密情報はSPIFFS/NVS）
│       └── infra/
│           ├── device.h/.cpp              device ID（MAC生成）・mqtt_host・cert CRC・job ID（NVS管理）
│           ├── lte.h/.cpp                 SIM7080G 制御・MQTT over TLS・SPIFFS 証明書読み込み
...
- **OTA ファームウェアアップデート**: **esp32_iot_gateway に実装済み**。
  AWS IoT Jobs で更新通知を受け取り、S3 公開バケットから HTTPS DL して書き込む。
  デュアルバンク（app0/app1）+ ロールバック（`CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE`）対応。
  詳細は `esp32_iot_gateway/OTA.md` を参照。
  m5atom_iot_gateway への適用は未対応（フラッシュサイズ・パーティション要確認）。
