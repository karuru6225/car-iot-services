...
5. **§4 テスト1（原因A/B切り分け）**: 診断用一時関数`canSendObdRequestTest2Pid()`
   ...
   診断用コードは削除せず残置（タスク2実装時に本実装へ置き換える前提）。

### 未着手

- **タスク2**: 多PID要求の送信対応（`canSendObdRequest` の複数PID版、物理アドレッシング）。
  §4テスト1で原因B確定済みのため実施する価値が確認できている。
- **タスク3**: 多PID応答パーサ（PID→データ長テーブルのTLV的パーサ）。

### 次にやること

1. タスク2（多PID送信対応）に着手。実装時に`canSendObdRequestTest2Pid()`
   （診断用・RPM+MAP固定）を汎用版に置き換える。
2. タスク3（多PID応答パーサ）に着手。
