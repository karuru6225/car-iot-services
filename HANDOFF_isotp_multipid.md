...
- **タスク3**: 多PID応答パーサ（PID→データ長テーブルのTLV的パーサ、`obd.cpp`）。
  `canSendObdRequestMulti()`の呼び出し元・実車での動作確認もここで行う想定。

### 次にやること

1. タスク3（多PID応答パーサ）に着手。`canSendObdRequestMulti()`を呼ぶ経路
   （まずは診断目的の簡易呼び出しでよい）を用意し、実車で多PID応答の
   パース結果を確認する。
2. （任意・実車検証）過給時のログを追加取得し、`iat_c`/`iat2_c`と
