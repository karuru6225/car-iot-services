...
7. **タスク3: 多PID応答パーサ**（`obd.h/.cpp`に`obdParseMultiResponse()`を追加）。
   ...
   `[TEST2]`タグでセグメントをログ出力。**実装のみ（実車未確認）**。
   ※本番統合（29PIDをSFに収まる組み合わせでバッチ化する`obdpoll.cpp`の書き換え）は
   スコープ外のまま。`kPidLengths[]`は今回テストしたRPM/MAP以外の27PID分は未検証。

### 未着手

- 本番統合（`obdpoll.cpp`の`kPids`ループをバッチ化し、実際に29PIDポーリングを
  高速化する）。§0進捗の6/7で「確度は高いが未検証」とした2点
  （`kPidLengths[]`の網羅性、PIDグループ分けロジック）をここで検証する。

### 次にやること

1. `isotp-multipid`ブランチを書き込み、実車で`[TEST2]`ログを確認:
   `pid=0x0C len=2 data=...`と`pid=0x0B len=1 data=...`が両方出るか。
   出れば送信API（タスク2）・TLVパーサ（タスク3）とも実車で動作確認完了。
2. 確認できたら診断用コード（`[TEST2]`まわり）を削除。
3. 本番統合に進むかどうかを判断（未着手セクション参照）。
4. （任意・実車検証）過給時のログを追加取得し、`iat_c`/`iat2_c`と
