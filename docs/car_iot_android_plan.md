...
Material3デフォルトのままだった配色を、アプリ内で切り替え可能な2テーマにする方針で
実装した（Flutter版のダークテーマ移植ではなく、独自の新規デザインを用意する方向）。

- `ui/theme/AppTheme.kt`: 2値のenum
- `ui/theme/ThemeStore.kt`: `SharedPreferences`で選択テーマを永続化
  （`meter/MeterConfigStore.kt`と同じパターン）。テーマ名を変えても、保存済みの旧名は
  `valueOf`のcatchで既定値へフォールバックするため移行処理は不要
- `MainActivity.kt`の`MaterialTheme(...)`に配線、接続タブ（`ConnectionScreen.kt`）に
  `FilterChip`2つの切り替えUIを追加

配色そのものはこの後3回作り直しており、最終的に計器盤ベースの「ナイト/デイ」に
着地している。経緯と設計判断は後述の「UIデザイン刷新」セクションを参照。
...
## Phase 9より先行実装: UIデザイン刷新（計器盤ベース）

上記のテーマ切り替えを入れた後、配色・レイアウトを**3回作り直している**。
最終形だけでなく捨てた案も残す。同じ回り道を繰り返さないため。

### 経緯：捨てた2案

1. **レーシング/ミニマル**（初版）: Material3の`ColorScheme`だけ差し替えた版。
   ウィジェットは標準のままなのでピル型ボタン・太いProgressIndicatorが残り、
   「Material3そのまま」の印象を脱していなかった。またミニマル側は
   「黄緑はワンポイントのみ」という設計意図に反して全ボタンが黄緑で塗られていた
2. **shadcn/ui(neutral)**: 公式の`:root`/`.dark`トークンをOKLCHから変換して忠実に
   移植した版。体系としては正しく動いたが、**Web管理画面のデザイン言語を車載計器に
   持ち込んだ結果、グレー一色の事務的なUIになり被写体と噛み合わなかった**

いずれも共通の失敗として、**「数値を見せる計器」ではなく「数値も出る設定アプリ」を
作っていた**。ラベル11sp・数値22〜30spと差が小さく、数値が主役になっていなかった。

### 現行：計器盤ベースの「ナイト/デイ」

- **タイポグラフィが設計の核**（`ui/theme/ClusterTypography.kt`）。実際の計器盤は
  極小ラベルと巨大数値の落差で出来ているため、ラベル10sp/字間+2.5sp ↔
  ヒーロー数値60sp/字間-2sp まで差を開いた。数値は等幅かつ細字
  （桁が動かず、大きくしても圧迫感が出ない。太字は使わない）
- **アクセントは暖色アンバー**（`ui/theme/ClusterColorSchemes.kt`）。夜間の自動車
  メーター照明が実際にこの色域という被写体由来の語彙であり、同時に
  「黒背景＋鮮やかな1色」という量産されがちな配色を避けられる。NIGHTの地色を
  純黒でなく青みのある`#06080C`にしたのも、クラスターがガラス越しに冷たく沈んで
  見えることに倣ったもの
...
- **ボタンの角丸は`ClusterButtonShape`を各所に明示指定**。Material3の`Button`は
  `Shapes`ではなく`CircleShape`固定で、テーマ側の`Shapes`差し替えだけでは
  ピル型のまま変わらない（ハマりどころ）

### Material 3 Expressiveを採用しなかった理由

Androidの現行デザイン言語（2025年5月発表、35種のシェイプライブラリとスプリング系
モーションが柱）だが、新コンポーネントAPIは`material3` 1.4.0+ が必要で、本プロジェクトは
1.3.2（`compose-bom` 2025.09.00）。依存追加は確認事項のため見送り、既存APIで組んだ。
将来BOMを上げる際に再検討の余地がある。

### エミュレータでのUI確認手法（ダミー値注入）

BLE接続できない環境では計測値が常にnullでプレースホルダー状態しか見られず、
デザインの良し悪しが判断できない。以下を一時的に書き換えて確認し、**確認後に必ず
`git checkout --`で戻す**運用を取った。
...
戻し忘れ防止として、注入行には必ず`// TODO: デザイン確認用ダミー値(戻すこと)`を付け、
撤去後に`grep -rn "デザイン確認用" app/src/`と`git status`の両方で確認する。

この手法で実際にバグを2件見つけている（円弧ゲージが描画ボックスをはみ出して下半分が
切れていた、単位が数値と同じ大きさで出てラベル/数値の落差が崩れていた）。
いずれもプレースホルダー状態では気づけないものだった。
